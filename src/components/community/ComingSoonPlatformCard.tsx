...
export default function ComingSoonPlatformCard({
  ...
  return (
    ...
    >
      ...
      {/* Vote link */}
      {hasVoted ? (
        <span className="font-body text-muted text-[12px] font-medium">✓ Voted</span>
      ) : (
        <button
          onClick={onVote}
          className="font-body text-accent text-[12px] font-semibold hover:text-ink transition-colors cursor-pointer"
        >
          Vote to prioritize →
        </button>
      )}
