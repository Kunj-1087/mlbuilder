...
export default function SavedPageClient() {
  ...
  const totalCount = user ? getBookmarkCount(user.id) : 0;
  const thisWeekCount = user ? getRecentBookmarkCount(user.id) : 0;
  const categoryCount = user ? getDistinctCategoryCount(user.id) : 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 md:py-16">
      ...
      {totalCount === 0 && activeFilter === 'all' && !searchQuery ? (
